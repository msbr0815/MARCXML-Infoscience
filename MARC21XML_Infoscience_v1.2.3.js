...
function doExport()
{
	...
	*/
	...
	//Run through all the items in the list until no more item is found
	while ((item = Z.nextItem()))
	{
		...
		if (typeOfPublication != "thesis")
		{
			if (typeOfPublication == "bill" && item.title == "Infoscience")
			{
				...
				/*
				 * I thought about filling the tempUnit fields with the final names already, but decided against it, as this might be an error source.
				 * At a first glance, the benefits from the reduced load due to the reduced amount of replacements would probably not justify the risk.
				 */
			}
			else
			{
